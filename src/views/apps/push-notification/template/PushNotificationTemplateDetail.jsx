...
const PushNotificationTemplateDetail = () => {
    ...
    const getTemplateDetail = useCallback(async () => {
        setIsLoading(true);
        const response = await apiClient.post('/api/push-notification/template/get-template-by-id', { id });
    ...
    const handleBack = () => {
        const url = getLocalizedUrl('/apps/push-notification', locale);
        router.push(url);
    }
        ...
        <Card>
            ...
            {!isLoading && (<CardContent className='flex flex-col pbs-12 gap-6'>
                <div>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <div className='flex items-center flex-wrap gap-x-1.5'>
                            ...
                            <Typography color='text.primary'>{templateData.title}</Typography>
                        </div>
                        <div className='flex items-center flex-wrap gap-x-1.5'>
                ...
                <div className='flex gap-4 justify-center'>
                    {((pushNotificationPermission?.editPermission === "Y")) && (
                        <OpenDialogOnElementClick
                            element={Button}
                            elementProps={buttonProps('Edit', 'primary', 'contained')}
                            dialog={EditTemplateInfo}
                            dialogProps={{ data: templateData, id: id, handleTemplateUpdate, emailPermission: pushNotificationPermission, flag: 'push-notification' }}
                        />
                    )}
...
export default PushNotificationTemplateDetail
