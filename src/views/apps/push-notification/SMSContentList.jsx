...
const SMSContentList = props => {
    ...
    const getSMS = useCallback(async (searchText) => {
        setIsLoading(true);
        try {
            const response = await apiClient.post("/api/push-notification/get-push-notification", {
                searchText,
                page: currentPage,
                pageSize
            });
    ...
    if (smsData.length === 0) {
        return (
            <div className='relative flex justify-center gap-2 grow is-full bg-backgroundPaper'>
                {isLoading ? <div className='relative overflow-hidden grow is-full mt-4'><Loader /></div> : <Typography className='m-3'>No Push Notification found!</Typography>}
                {reload && (
