...
const ComposeSMS = props => {
    const { openCompose, setOpenCompose, isBelowSmScreen, isBelowMdScreen, setSMSDeliveredStatus, pushNotificationPermission } = props;
    ...
    const getTemplates = useCallback(async () => {
        const response = await apiClient.post("/api/push-notification/template/get-templates", {});
    ...
    const handleTemplateChange = (event) => {
        ...
        if (templates?.length > 0) {
            ...
            if (templateContent && editor) {
                ...
                setTitle(templateContent.title);
    ...
    const handleSend = async () => {
        ...
        const postData = {
            ...
            title: title ?? '',
            message: message ?? '',
            sendPushNotificationTo: recipientType,
            templateId: templateId ?? null,
            templateCategory: templateCategory ?? ''
        };
        try {
            const response = await apiClient.post('/api/push-notification', postData);
            if (response.data.result === true) {
                setApiErrors({});
    ...
    return (
        ...
        >
            <div className='flex items-center justify-between plb-3 pli-6 bg-actionHover'>
                <Typography variant='h5' color='text.secondary'>Send Push Notification</Typography>
            ...
            {apiErrors?.noTokens && (
                <div className='plb-3 pli-6'>
                    <Typography variant="body2" color="error" className="mt-1">{apiErrors.noTokens}</Typography>
                </div>
            )}
            ...
            <div className='plb-3 pli-6'>
                <FormControl component="fieldset">
                    <div className="flex items-center">
                        <FormLabel component="legend" className="mr-3">Send push notification to :</FormLabel>
            ...
            {recipientType === 'single-user' && (
                <>
                    <div className='flex items-center gap-2 pli-6 plb-1 border-bs'>
                        <Typography className='font-medium' color='text.disabled'>To:</Typography>
                        <InputBase fullWidth value={to} onChange={e => setTo(e.target.value)} placeholder='Ex: admin@gmail.com' />
                        {apiErrors?.to && (
            ...
            <div className='flex items-center gap-2 pli-6 plb-1'>
                <Typography className='font-medium' color='text.disabled'>Title:</Typography>
                <InputBase fullWidth value={title} onChange={e => setTitle(e.target.value)} placeholder='News letter' />
                {apiErrors?.title && (
                    <Typography variant="body2" color="error" className="mt-1">{apiErrors.title}</Typography> // Display error message
                )}
            </div>
...
ComposeSMS.propTypes = {
    ...
    pushNotificationPermission: PropTypes.any
}
