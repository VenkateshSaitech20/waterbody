...
const VoiceCallContentList = props => {
    const { isInitialMount, isBelowSmScreen, isBelowLgScreen, reload, searchTerm, setDrawerOpen, setCurrentSMS, voiceCallDeliveredStatus } = props;
    ...
    const getSMS = useCallback(async (searchText) => {
        setIsLoading(true);
        try {
            const response = await apiClient.post("/api/voice-call/get-voice-calls", {
                searchText,
                page: currentPage,
                pageSize
            });
    ...
    const handleVoiceCallClick = sms => {
        setDrawerOpen(true);
        setCurrentSMS(sms);
    }
    ...
    useEffect(() => {
        if (voiceCallDeliveredStatus === true) {
            getSMS('');
        }
    }, [getSMS, voiceCallDeliveredStatus]);
    ...
    if (smsData.length === 0) {
        return (
            <div className='relative flex justify-center gap-2 grow is-full bg-backgroundPaper'>
                {isLoading ? <div className='relative overflow-hidden grow is-full mt-4'><Loader /></div> : <Typography className='m-3'>No call found!</Typography>}
                {reload && (
                            ...
                            <div className='flex flex-col'>
                                {smsData && smsData.map(sms => (
                                    <div
                                        ...
                                        onClick={() => handleVoiceCallClick(sms)}
                                    >
...
VoiceCallContentList.propTypes = {
    ...
    voiceCallDeliveredStatus: PropTypes.bool
};
...
export default VoiceCallContentList;
