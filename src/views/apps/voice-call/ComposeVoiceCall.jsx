...
const ComposeVoiceCall = props => {
    const { openCompose, setOpenCompose, isBelowSmScreen, isBelowMdScreen, setVoiceCallDeliveredStatus, voiceCallPermission } = props;
    ...
    const handleSend = async () => {
        ...
        setVoiceCallDeliveredStatus(false);
        let message = editor.getHTML();
        message = message.replace(/<p>|<\/p>/g, '').trim();
        const postData = {
            to: to ?? '',
            message: message ?? '',
            sendVoiceCallTo: recipientType,
            templateId: templateId ?? null,
            templateCategory: templateCategory ?? ''
        };
        try {
            const response = await apiClient.post('/api/voice-call', postData);
            if (response.data.result === true) {
                setApiErrors({});
                showToast(true, response.data.message);
                setIsButtonLoading(false);
                setOpenCompose(false);
                setVoiceCallDeliveredStatus(true);
            } else if (response.data.result === false) {
                setVoiceCallDeliveredStatus(false);
                if (response?.data?.message?.roleError?.name === responseData.tokenExpired || response?.data?.message?.invalidToken === responseData.invalidToken) {
                    await signOut({ callbackUrl: process.env.NEXT_PUBLIC_APP_URL });
    ...
    return (
        ...
        >
            <div className='flex items-center justify-between plb-3 pli-6 bg-actionHover'>
                <Typography variant='h5' color='text.secondary'>Voice Call</Typography>
            ...
            <div className='plb-3 pli-6'>
                <FormControl component="fieldset">
                    <div className="flex items-center">
                        <FormLabel component="legend" className="mr-3">Send voice call to :</FormLabel>
                ...
                {((voiceCallPermission?.editPermission === "Y" || voiceCallPermission?.writePermission === "Y")) && (
                    <div className='flex items-center gap-4 max-sm:gap-3'>
                        {isBelowSmScreen ? (
                            ...
                            <Button variant='contained' endIcon={isButtonLoading ? '' : <i className='bx-send' />} onClick={handleSend} disabled={isButtonLoading}>{isButtonLoading ? <Loader type="btnLoader" /> : 'Call'}</Button>
                        )}
