...
const ComposeSMS = props => {
    const { openCompose, setOpenCompose, isBelowSmScreen, isBelowMdScreen, setSMSDeliveredStatus, smsPermission } = props;
    ...
    const handleSend = async () => {
        ...
        const postData = {
            to: to ?? '',
            message: message ?? '',
            sendSMSTo: recipientType,
            templateId: templateId ?? null,
            templateCategory: templateCategory ?? ''
        };
        try {
            const response = await apiClient.post('/api/sms', postData);
            if (response.data.result === true) {
                setApiErrors({});
    ...
    return (
        ...
        >
            <div className='flex items-center justify-between plb-3 pli-6 bg-actionHover'>
                <Typography variant='h5' color='text.secondary'>Send SMS</Typography>
            ...
            <div className='plb-3 pli-6'>
                <FormControl component="fieldset">
                    <div className="flex items-center">
                        <FormLabel component="legend" className="mr-3">Send sms to :</FormLabel>
