...
const SMSTemplateTable = ({ smsPermission }) => {
    ...
    const handleViewProfile = (id) => {
        const url = getLocalizedUrl(`/apps/sms/template/${id}`, locale)
        router.push(url);
    };
    const { lang: locale } = useParams();
    const getTemplates = useCallback(async (searchText) => {
        setIsLoading(true);
        const response = await apiClient.post("/api/sms/template/get-templates", {
            searchText,
            page: currentPage,
            pageSize
        });
    ...
    const handleConfirmation = async (confirmed) => {
        setIsLoading(true);
        if (confirmed) {
            const response = await apiClient.put('/api/sms/template', { id: idToDelete });
            if (response.data.result === true) {
                await getTemplates(searchText);
                setOpen(false);
            } else if (response.data.result === false) {
            ...
            columnHelper.accessor('action', {
                ...
                cell: ({ row }) => (
                    <div className='flex items-center'>
                        {((smsPermission?.deletePermission === "Y")) && (
                            <IconButton onClick={() => handleDelete(row.original.id)}>
                                <i className='bx-trash text-textSecondary text-[22px]' />
                            </IconButton>
                        )}
                        {((smsPermission?.readPermission === "Y")) && (
                ...
                <CardContent className='flex justify-between flex-wrap max-sm:flex-col sm:items-center gap-4'>
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        <TextFieldStyled
                            ...
                            variant='filled'
                        />
                        <Button variant='tonal' color='primary' onClick={handleSMSTemplateSearch}>
                            {isLoading ? <Loader type="btnLoader" /> : 'Search'}
                        </Button>
                    </div>
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        {((smsPermission?.writePermission === "Y")) && (
                            <Button
                                variant='contained'
