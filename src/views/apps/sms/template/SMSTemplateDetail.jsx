...
const SMSTemplateDetail = () => {
    ...
    const getTemplateDetail = useCallback(async () => {
        setIsLoading(true);
        const response = await apiClient.post('/api/sms/template/get-template-by-id', { id });
    ...
    const handleBack = () => {
        const url = getLocalizedUrl('/apps/sms', locale);
        router.push(url);
    }
        ...
        <Card>
            ...
            {!isLoading && (<CardContent className='flex flex-col pbs-12 gap-6'>
                ...
                <div className='flex gap-4 justify-center'>
                    {((smsPermission?.editPermission === "Y")) && (
                        <OpenDialogOnElementClick
                            element={Button}
                            elementProps={buttonProps('Edit', 'primary', 'contained')}
                            dialog={EditTemplateInfo}
                            dialogProps={{ data: templateData, id: id, handleTemplateUpdate, emailPermission: smsPermission, flag: 'sms' }}
                        />
                    )}
