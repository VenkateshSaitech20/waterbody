...
const WhatsAppContentList = props => {
    ...
    const getSMS = useCallback(async (searchText) => {
        setIsLoading(true);
        try {
            const response = await apiClient.post("/api/whats-app/get-whats-app", {
                searchText,
                page: currentPage,
                pageSize
            });
            if (response.data.result === true) {
                setWhatsAppData(response.data.message);
                    ...
                    (
                        <ScrollWrapper isBelowLgScreen={isBelowLgScreen}>
                            <div className='flex flex-col'>
                                {whatsAppData && whatsAppData.map(sms => (
                                    <div
                                        key={sms.id}
...
export default WhatsAppContentList;
