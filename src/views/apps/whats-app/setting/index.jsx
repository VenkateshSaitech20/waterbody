...
import { registerData, responseData, whatsAppType } from '@/utils/message';
...
const SMSSetting = () => {
    ...
    const handleOnSubmit = async (data) => {
        ...
        if (id) data.id = id;
        const response = await apiClient.post('/api/whats-app/setting', data);
    ...
    const getWhatsAppSetting = async () => {
        setIsLoading(true);
        const response = await apiClient.get('/api/whats-app/setting');
        if (response.data.result === true) {
                        ...
                        <Grid container spacing={6}>
                            <Grid item xs={12} sm={6}>
                                <Controller
                                    name='whatsAppType'
                                    control={control}
                                    rules={{ required: registerData.whatsAppTypeReq }}
                                    defaultValue=''
                                    render={({ field }) => (
                                        <TextFieldStyled
                                            select
                                            ...
                                            size={"small"}
                                            id='whatsapp-type'
                                            label={<CustomInputLabel htmlFor='whatsapp-type' text='WhatsApp Type' />}
                                            {...field}
                                            error={Boolean(errors?.whatsAppType) || !!apiErrors?.whatsAppType}
                                            helperText={errors?.whatsAppType?.message || apiErrors?.whatsAppType}
                                        >
                                            {whatsAppType?.map(type => (
