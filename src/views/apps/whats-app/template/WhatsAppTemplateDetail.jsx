...
const WhatsAppTemplateDetail = () => {
    ...
    const getTemplateDetail = useCallback(async () => {
        setIsLoading(true);
        const response = await apiClient.post('/api/whats-app/template/get-template-by-id', { id });
    ...
    const handleBack = () => {
        const url = getLocalizedUrl('/apps/whats-app', locale);
        router.push(url);
    }
        ...
        <Card>
            ...
            {!isLoading && (<CardContent className='flex flex-col pbs-12 gap-6'>
                ...
                <div className='flex gap-4 justify-center'>
                    {((whatsAppPermission?.editPermission === "Y")) && (
                        <OpenDialogOnElementClick
                            element={Button}
                            elementProps={buttonProps('Edit', 'primary', 'contained')}
                            dialog={EditTemplateInfo}
                            dialogProps={{ data: templateData, id: id, handleTemplateUpdate, emailPermission: whatsAppPermission, flag: 'whats-app' }}
                        />
                    )}
