...
const WhatsAppTemplateTable = ({ whatsAppPermission }) => {
    ...
    const handleViewProfile = (id) => {
        const url = getLocalizedUrl(`/apps/whats-app/template/${id}`, locale)
        router.push(url);
    };
    ...
    const handleConfirmation = async (confirmed) => {
        setIsLoading(true);
        if (confirmed) {
            const response = await apiClient.put('/api/whats-app/template', { id: idToDelete });
            if (response.data.result === true) {
                await getTemplates(searchText);
                setOpen(false);
            } else if (response.data.result === false) {
            ...
            columnHelper.accessor('action', {
                ...
                cell: ({ row }) => (
                    <div className='flex items-center'>
                        {((whatsAppPermission?.deletePermission === "Y")) && (
                            <IconButton onClick={() => handleDelete(row.original.id)}>
                                <i className='bx-trash text-textSecondary text-[22px]' />
                            </IconButton>
                        )}
                        {((whatsAppPermission?.readPermission === "Y")) && (
    ...
    const handleWhatsAppTemplateSearch = () => {
        getTemplates(searchText);
    };
    return (
        <>
                ...
                <CardContent className='flex justify-between flex-wrap max-sm:flex-col sm:items-center gap-4'>
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        <TextFieldStyled
                            ...
                            variant='filled'
                        />
                        <Button variant='tonal' color='primary' onClick={handleWhatsAppTemplateSearch}>
                            {isLoading ? <Loader type="btnLoader" /> : 'Search'}
                        </Button>
                    </div>
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        {((whatsAppPermission?.writePermission === "Y")) && (
                            <Button
                                variant='contained'
...
WhatsAppTemplateTable.propTypes = {
