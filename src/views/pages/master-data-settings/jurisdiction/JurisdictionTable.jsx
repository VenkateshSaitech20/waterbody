...
import JurisdictionDialog from '@/components/dialogs/edit-jurisdiction-info';
...
const JurisdictionTable = ({ masterSettingsPermission }) => {
    ...
    const getJurisdictions = useCallback(async (searchText) => {
        setIsLoading(true);
        const response = await apiClient.post("/api/master-data-settings/jurisdiction/list", {
            searchText,
            page: currentPage,
            pageSize
        });
    ...
    const handleConfirmation = async (confirmed) => {
        setIsLoading(true);
        if (confirmed) {
            const response = await apiClient.put('/api/master-data-settings/jurisdiction', { id: idToDelete });
            if (response.data.result === true) {
                await getJurisdictions(searchText);
                setOpen(false);
            } else if (response.data.result === false) {
                if (response?.data?.message?.roleError?.name === responseData.tokenExpired || response?.data?.message?.invalidToken === responseData.invalidToken) {
                    await signOut({ callbackUrl: process.env.NEXT_PUBLIC_APP_URL });
    ...
    return (
        <>
            <Card>
                ...
                />
                <ConfirmationDialog open={open} setOpen={setOpen} type='delete-data' onConfirm={handleConfirmation} name="jurisdiction" />
            </Card>
            <AddJurisdiction
                open={dialogBoxOpen}
