...
import UrbanLocalBodyDialog from '@/components/dialogs/edit-urban-local-body-info';
import { pageList, responseData } from '@/utils/message';
import apiClient from '@/utils/apiClient';
import { signOut } from 'next-auth/react';
import Chip from '@mui/material/Chip'
import AddUrbanLocalBodies from './AddUrbanLocalBodies';
...
const UrbanLocalBodiesTable = ({ masterSettingsPermission }) => {
    ...
    const getLocalBodies = useCallback(async (searchText) => {
        setIsLoading(true);
        const response = await apiClient.post("/api/master-data-settings/urban-local-bodies/list", {
            searchText,
            page: currentPage,
            pageSize
        });
    ...
    const handleConfirmation = async (confirmed) => {
        setIsLoading(true);
        if (confirmed) {
            const response = await apiClient.put('/api/master-data-settings/urban-local-bodies', { id: idToDelete });
            if (response.data.result === true) {
                await getLocalBodies(searchText);
                setOpen(false);
            } else if (response.data.result === false) {
                if (response?.data?.message?.roleError?.name === responseData.tokenExpired || response?.data?.message?.invalidToken === responseData.invalidToken) {
                    await signOut({ callbackUrl: process.env.NEXT_PUBLIC_APP_URL });
    ...
    const columns = useMemo(() => {
        ...
        const baseColumns = [
            ...
            columnHelper.accessor('wardCode', {
                header: 'Ward Code',
                cell: ({ row }) => (
                    <div className='flex items-center gap-2'>
                        <Icon />
                        <Typography color='text.primary'>
                            {row.original.wardCode}
                        </Typography>
                    </div>
                )
            }),
    ...
    return (
        <>
            <Card>
                ...
                />
                <ConfirmationDialog open={open} setOpen={setOpen} type='delete-data' onConfirm={handleConfirmation} name="data" />
            </Card>
            <AddUrbanLocalBodies
                open={addDialogOpen}
                handleClose={() => setAddDialogOpen(!addDialogOpen)}
...
UrbanLocalBodiesTable.propTypes = {
    masterSettingsPermission: PropTypes.any,
}

export default UrbanLocalBodiesTable
