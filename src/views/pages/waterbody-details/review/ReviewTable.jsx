...
const ReviewTable = ({ reviewPermission }) => {
    ...
    const getReviews = useCallback(async (searchFilter) => {
        setIsLoading(true);
        const response = await apiWBDClient.post("water-body-details/review/get", {
            searchFilter,
            page: currentPage,
            pageSize
        });
    ...
    const handleViewPage = (id) => {
        const url = getLocalizedUrl(`/pages/waterbody-details/review/${id}`, locale)
        router.push(url);
    };
    ...
    const columns = useMemo(() => {
        const canDelete = reviewPermission?.deletePermission === 'Y';
        const canEdit = reviewPermission?.editPermission === 'Y';
        const baseColumns = [
        ...
        return baseColumns;
    }, [data, filteredData, reviewPermission]);
...
ReviewTable.propTypes = {
    reviewPermission: PropTypes.any,
}
