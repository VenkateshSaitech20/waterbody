...
const Block = () => {
    ...
    const handleOnSubmit = async (data) => {
        ...
        const response = await apiClient.post('/api/bulk-import/import-blocks', formData, {
                        ...
                        <div className='flex flex-col items-start'>
                            <form onSubmit={handleSubmit(handleOnSubmit)} className='w-full'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    <Button size="small" variant='contained' style={{ width: '50%' }} startIcon={<i className='bx-download' />} onClick={() => handleExport('blocks')}> {(isDwnldBtnLoading) ? <Loader type="btnLoader" /> : "Download"}</Button>
                                </div>
                            </form>
                            {apiErrors?.blockErr && <Typography className='text-red-500 mt-2'>{apiErrors.blockErr}</Typography>}
                        </div>
                    </div>
                </CardContent>
            </Card>
