...
const Habitation = () => {
    ...
    const handleOnSubmit = async (data) => {
        ...
        formData.append('panchayatId', data.panchayatId)
        const response = await apiClient.post('/api/bulk-import/import-habitations', formData, {
                        ...
                        <div className='flex flex-col items-start'>
                            <form onSubmit={handleSubmit(handleOnSubmit)} className='w-full'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    <Button size="small" variant='contained' style={{ width: '50%' }} startIcon={<i className='bx-download' />} onClick={() => handleExport('habitations')}> {(isDwnldBtnLoading) ? <Loader type="btnLoader" /> : "Download"}</Button>
                                </div>
                            </form>
                            {apiErrors?.habitationErr && <Typography className='text-red-500 mt-2'>{apiErrors.habitationErr}</Typography>}
                        </div>
                    </div>
                </CardContent>
            </Card>
