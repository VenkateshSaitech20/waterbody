...
const VolunteersReport = () => {
    ...
    const getVolunteersReports = useCallback(async (searchText) => {
        if (isLoading === true) { return }
        setIsLoading(true);
        const response = await apiWBDClient.post("/volunteer/get", {
            searchText,
            page: currentPage,
            pageSize,
        });
    ...
    useEffect(() => {
        getVolunteersReports(searchText);
    }, [getVolunteersReports]);

    const handleExport = async () => {
        if (isButtonLoading === true) { return }
        try {
            setIsButtonLoading(true);
            const response = await apiWBDClient.post('/volunteer/bulk-download', { searchText }, { responseType: 'blob' });
    ...
    const columns = useMemo(
        () => [
            ...
            columnHelper.accessor('mobile', {
                ...
                cell: ({ row }) => (
                    <div><Typography>{row.original.mobile}</Typography></div>
                )
            }),
            columnHelper.accessor('volunteeringFor', {
                header: 'volunteering For',
                cell: ({ row }) => (
                    <div className='capitalize'><Typography>{row.original.volunteeringFor}</Typography></div>
                )
            }),
            columnHelper.accessor('taluk', {
                header: 'taluk',
                cell: ({ row }) => <Typography className='capitalize'>{row.original.taluk}</Typography>
            }),
            columnHelper.accessor('block', {
                header: 'block',
                cell: ({ row }) => <Typography className='capitalize'>{row.original.block}</Typography>
            })
        ],
        [data, filteredData]
    )
...
export default VolunteersReport
