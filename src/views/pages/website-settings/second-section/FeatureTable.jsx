...
const FeatureTable = ({ websiteSettingsPermission }) => {
    ...
    const handleViewProfile = (id) => {
        const url = getLocalizedUrl(`/pages/website-settings/second-section/${id}`, locale)
        router.push(url);
    };
    ...
    const getFeatures = useCallback(async (searchText) => {
        setIsLoading(true);
        const response = await apiClient.post("/api/website-settings/second-section/list", {
            searchText,
            page: currentPage,
            pageSize
        });
    ...
    const handleFeatureAdded = async () => {
        await getFeatures(searchText);
    };

    useEffect(() => {
        getFeatures(searchText);
    }, [getFeatures]);

    const handleConfirmation = async (confirmed) => {
        setIsLoading(true);
        if (confirmed) {
            const response = await apiClient.put('/api/website-settings/second-section', { id: idToDelete });
            if (response.data.result === true) {
                await getFeatures(searchText);
                setOpen(false);
            } else if (response.data.result === false) {
    ...
    return (
        <>
            <Card>
                <CardContent className='flex justify-between flex-wrap max-sm:flex-col sm:items-center gap-4'>
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        <TextFieldStyled
                            ...
                            variant='filled'
                        />
                        <Button variant='tonal' color='primary' onClick={handleFeatureSearch}>
                    ...
                    <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
                        {websiteSettingsPermission && websiteSettingsPermission?.writePermission === "Y" && (
                            <Button
                                ...
                                onClick={() => setAddFeatureOpen(!addFeatureOpen)}
                ...
                />
                <ConfirmationDialog open={open} setOpen={setOpen} type='delete-feature' onConfirm={handleConfirmation} />
            </Card>
            <AddFeature
                open={addFeatureOpen}
                handleClose={() => setAddFeatureOpen(!addFeatureOpen)}
                handleFeatureAdded={handleFeatureAdded}
            />
        </>
    )
}
