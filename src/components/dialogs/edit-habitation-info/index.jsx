...
const HabitationDialog = ({ open, setOpen, data, id, handleDataUpdate }) => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            fetchBlocksByTalukId(data?.talukId);
            fetchPanchayatsByBlockId(data?.blockId);
            reset({
                name: data?.name || '',
    ...
    const handleOnSubmit = async (data) => {
        setIsLoading(true);
        if (id) data.id = id;
        const { data: { result, message } } = await apiClient.post('/api/master-data-settings/habitation', data);
        if (result === true) {
                ...
                <DialogContent className='overflow-visible pbs-0 sm:pli-16'>
                    <Grid container spacing={5}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextFieldStyled
                                fullWidth
                                label={<CustomInputLabel htmlFor='habitationName' text='Habitation Name' />}
                                size="small"
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextFieldStyled
                                fullWidth
                                label={<CustomInputLabel htmlFor='HabitationCode' text='Habitation Code' />}
                                size="small"
                                placeholder='Eg: 25'
                                InputLabelProps={{ shrink: true }}
                                variant='filled'
...
export default HabitationDialog
