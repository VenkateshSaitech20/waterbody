...
export async function POST(req) {
    try {
        ...
        if (page && pageSize) {
            ...
            const totalCount = await prisma.sms_templates.count({ where: queryFilter });
            if (skip >= totalCount) {
                skip = totalCount - pageSize
            }
            if (skip < 0) skip = 0
            const totalPages = Math.ceil(totalCount / pageSize);
            const templates = await prisma.sms_templates.findMany({ where: queryFilter, skip, take: pageSize, orderBy: { id: 'desc' } });
            if (templates) {
