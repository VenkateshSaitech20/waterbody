...
export async function POST(req) {
    ...
    try {
        ...
        const isCategoryExist = await prisma.sms_templates.findFirst({ where: { createdBy: user.id, slugCategory: slugCategory, isDeleted: "N", ...(id ? { NOT: { id: parseInt(id) } } : {}) } });
        if (isCategoryExist) {
        ...
        if (id) {
            ...
            const updatedTemplateData = await prisma.sms_templates.update({ where: { id: parseInt(id) }, data: fields });
            return NextResponse.json({ result: true, message: updatedTemplateData });
        } else {
            fields.createdBy = userId;
            await prisma.sms_templates.create({ data: fields });
