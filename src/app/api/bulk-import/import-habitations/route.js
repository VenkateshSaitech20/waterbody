...
export async function POST(req) {
    try {
        ...
        const habitations = xlsx.utils.sheet_to_json(sheet);
        ...
        const panchayatId = formData.get('panchayatId');
        ...
        const panchayatData = await prisma.panchayats.findUnique({ where: { id: +panchayatId } });
        if (!countryData || !stateData || !districtData || !talukData || !blockData || !panchayatData) {
            return NextResponse.json({ result: false, message: { habitationErr: responseData.habitationValidFieldErr } });
        }
        const createHabitationPromises = habitations.map(async (habitation) => {
            if (!habitation.name || !habitation.lgdCode) {
                return { result: false, message: { habitationErr: responseData.habitationMissingFieldErr } };
            }
            let slug = generateSlug(habitation.name);
            const existingHabitation = await prisma.habitations.findFirst({ where: { slug: slug } });
            if (existingHabitation) {
                const habitationId = existingHabitation.id;
                await prisma.habitations.update({
                    where: { id: habitationId },
                    data: {
                        name: capitalizeFirstLetter(habitation.name),
                        slug: slug,
                        lgdCode: String(habitation.lgdCode),
                        ...
                        panchayatId: panchayatData.id,
                        panchayat: capitalizeFirstLetter(panchayatData.name)
                    },
                });
            } else {
                await prisma.habitations.create({
                    data: {
                        name: capitalizeFirstLetter(habitation.name),
                        slug: slug,
                        lgdCode: String(habitation.lgdCode),
            ...
            return { result: true, message: responseData.habitationsUpdated };
        });
        const results = await Promise.all(createHabitationPromises);
        ...
        return NextResponse.json({ result: true, message: responseData.habitationsUpdated });
    } catch (error) {
