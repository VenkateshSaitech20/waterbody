    ...
    try {
        ...
        const queryFilter = {
            AND: [
                ...
                ...(searchText ? [{
                    OR: [
                        { keyMetric: { contains: searchText } },
                        { highlight: { contains: searchText } },
        ...
        const totalCount = await prisma.key_achievements.count({
            where: queryFilter
        });
        if (skip >= totalCount) {
        ...
        const users = await prisma.key_achievements.findMany({
            where: queryFilter,
            select: {
                id: true,
                image: true,
