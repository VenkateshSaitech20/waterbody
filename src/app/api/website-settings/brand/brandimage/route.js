...
export async function GET() {
    try {
        const brandSection = await prisma.brands.findFirst({});
        if (brandSection) {
...
export async function DELETE(req) {
    try {
        ...
        const { id } = body;
        const existingBrandDetails = await prisma.brands.findUnique({ where: { id } });
        if (!existingBrandDetails) {
            return NextResponse.json({ result: false, message: { notFound: responseData.notFound } });
        }
        ...
        for (const key of imageKeys) {
            ...
            if (oldImageUrl) {
                ...
                try {
                    await deleteFile(oldFilePath);
                    console.log(`File deleted at: ${oldFilePath}`);
                } catch (error) {
        ...
        return NextResponse.json({ result: true, message: responseData.brandDeleted });
    } catch (error) {
        return NextResponse.json({ result: false, message: error.message });
    }
}
