...
export async function POST(req) {
    try {
        ...
        const totalCount = await prisma.sent_voice_call.count({ where: queryFilter });
        if (skip >= totalCount) {
            skip = totalCount - pageSize;
        }
        if (skip < 0) skip = 0;
        const totalPages = Math.ceil(totalCount / pageSize);
        const sms = await prisma.sent_voice_call.findMany({ where: queryFilter, skip, take: pageSize, orderBy: { id: 'desc' } });
        if (sms) {
