...
export async function POST(req) {
    try {
        ...
        if (page && pageSize) {
            ...
            const totalCount = await prisma.whats_app_templates.count({ where: queryFilter });
            if (skip >= totalCount) {
                skip = totalCount - pageSize
            }
            if (skip < 0) skip = 0
            const totalPages = Math.ceil(totalCount / pageSize);
            const templates = await prisma.whats_app_templates.findMany({ where: queryFilter, skip, take: pageSize, orderBy: { id: 'desc' } });
            if (templates) {
            ...
            // For dropdown list
            const templates = await prisma.whats_app_templates.findMany({
                where: {
                    createdBy: userId,
                    isDeleted: 'N',
                    isActive: 'Y'
