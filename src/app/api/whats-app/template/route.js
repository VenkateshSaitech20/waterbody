...
export async function POST(req) {
    try {
        ...
        const isCategoryExist = await prisma.whats_app_templates.findFirst({ where: { createdBy: user.id, slugCategory: slugCategory, isDeleted: "N", ...(id ? { NOT: { id: parseInt(id) } } : {}) } });
        if (isCategoryExist) {
        ...
        if (id) {
            ...
            const updatedTemplateData = await prisma.whats_app_templates.update({ where: { id: parseInt(id) }, data: fields });
            return NextResponse.json({ result: true, message: updatedTemplateData });
        } else {
            fields.createdBy = userId;
            await prisma.whats_app_templates.create({ data: fields });
...
export async function PUT(req) {
    try {
        ...
        const body = await req.json();
        const { id } = body;
        await prisma.whats_app_templates.update({
            where: { id: id },
